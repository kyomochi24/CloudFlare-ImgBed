...
## 第七次更新：丘丘智能抠图、切切蛋糕和首次公告

这一版在原有第六次更新基础上安装。请把「浅浅丘的甜品屋-第七次更新.zip」**解压**，按原有文件夹路径覆盖到 GitHub 仓库，然后提交并推送。特别注意要把 `frontend-dist/studio/models/u2netp.onnx` 和 `frontend-dist/studio/vendor/ort/ort-wasm-simd-threaded.wasm` 这两个大文件一起上传；每个文件都小于 Cloudflare Pages 的 25 MiB 单文件上限。**不要把 zip 压缩包本身上传到 GitHub 仓库。**

部署新代码前，请到 Cloudflare → D1 数据库 → `qiuqiu_d1`（如果你的 D1 叫别的名字，就选原来绑定 `img_d1` 的那个）→ 控制台，执行 [`database/migrations/studio-cutout.sql`](database/migrations/studio-cutout.sql) 里的完整 SQL。只需执行一次；它新增每日抠图次数表，不会删除相册和图片。执行后可输入 `SELECT name FROM sqlite_master WHERE name='studio_cutout_usage';` 检查，看到表名即成功。

部署后刷新 `/studio/`：

1. 先登录，顶部会出现「丘丘智能抠图」「切切蛋糕」入口。未登录看不到工具，抠图次数接口也会拒绝访问。
2. 丘丘智能抠图可以点击或拖入图片；皮丘每天 10 张、每次最多 2 张；皮卡丘每天 30 张、每次最多 4 张；超级无敌美化大师丘每天 30 张、每次最多 8 张。每日按北京时间 00:00 重置。打开模型后点击「开始抠图」才计次；开始处理后即使中途关闭网页也会占用次数。
3. 首次抠图需从你自己的网站加载约 4.6 MB 的模型和约 11 MB 的运行时，手机可能慢一些。图片在浏览器本地抠图；点击「保存到当前相册」才上传到 R2 并按原有相册存储额度计费。也可以直接下载透明 PNG。浏览器版用轻量 U²-NetP，边缘效果可能不如 Jevet 的大型桌面模型。
4. 切切蛋糕选择一张图片后，点击「＋竖线」「＋横线」，拖动圆形手柄调整位置，也可填横向和纵向份数再点「等距分割」。确认裁剪后，取消勾选不想保留的切片；保存时只把选中的图片上传到自动创建的「分割相册」。四条竖线会得到五张。每个方向最多 20 条线，总切片最多 80 张。这个工具本身不消耗抠图次数；保存的图片仍按原有相册额度计算。
5. 站长到 `/studio/admin.html` 编辑公告，可以先写文字，再选中其中一段，选文字颜色、背景色、字号，点击「应用到选中文字」，预览后保存。每个新版公告会在访客首次打开网站时自动弹出一次；看完关闭后可随时从顶部「公告」按钮再打开。公告图片功能照旧。

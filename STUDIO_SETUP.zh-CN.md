...
7. 在相册里点击「上传原图」选择图片，可以一次选择多张。上传时进度条会按浏览器实际发送的字节更新；服务器保存完成才显示 100%。点击图片可依次选中并显示 1、2、3 等编号，然后一键复制已选图链。图片卡片上的「重命名」可以修改相册里显示的文件名；原图链不会改变。美化搬家助手可选择或拖入 `.json`，也可点击「清空当前选择」重新选文件；把里面的图片逐张导入当前相册，再点「下载替换后的 JSON」。你可以在「搬家后的美化名称」框中改名；新名称会写入 JSON 顶层 `name`，并作为下载文件名。原文件保持不变；某条图链搬运失败时，新 JSON 中该条图链仍使用旧地址。普通用户单张图片最大 25 MiB，单个 JSON 最大 5 MiB。

(() => {
  ...
  function render(data) {
    ...
    $('user-list').innerHTML = data.users.length ? data.users.map(u => `<div class="admin-row"><div><strong>${escapeHtml(u.discord_name || u.username || u.id)}</strong><span class="tag">${u.tier === 'pikachu' ? '皮卡丘' : '皮丘'}</span>${u.disabled ? '<span class="tag">已停用</span>' : ''}<p>${u.account_type === 'discord' ? 'Discord' : '专用账号'} · ${escapeHtml(u.discord_id || u.username || '')} · 已存 ${fmt(u.stored_bytes)} · 本月上传 ${fmt(u.month_uploaded_bytes)}</p></div><div class="admin-actions"><button data-user="${escapeHtml(u.id)}" data-tier="${u.tier === 'pikachu' ? 'pichu' : 'pikachu'}">设为${u.tier === 'pikachu' ? '皮丘' : '皮卡丘'}</button><button data-user="${escapeHtml(u.id)}" data-disabled="${u.disabled ? 'false' : 'true'}">${u.disabled ? '启用' : '停用'}</button></div></div>`).join('') : '<div class="admin-empty">还没有用户</div>';
  }
